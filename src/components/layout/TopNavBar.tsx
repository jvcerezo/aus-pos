...
export const TopNavBar: React.FC<{ onOpenPinModal: () => void }> = ({ onOpenPinModal }) => {
  ...
  return (
    <header className="h-16 bg-white text-slate-900 px-4 flex items-center justify-between select-none border-b border-slate-200 z-30 shadow-xs">
      {/* Left: Venue Switcher & ABN (Larger Text) */}
      <div className="flex items-center space-x-2">
        <div className="relative">
          <button
            onClick={() => setIsVenueDropdownOpen(!isVenueDropdownOpen)}
            className="flex items-center space-x-2.5 bg-slate-100 hover:bg-slate-200/80 px-3.5 py-2 rounded-xl border border-slate-200 transition"
          >
            <div
              ...
              <div className="absolute top-full left-0 mt-1 w-72 bg-white border border-slate-200 rounded-xl shadow-xl p-1.5 z-40 space-y-1">
                ...
                {venues.map(v => (
                  <button
                    ...
                    className={`w-full flex items-center justify-between p-2.5 rounded-lg transition text-left text-xs ${
                      v.id === activeVenue.id
                  ...
                  >
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-slate-500 capitalize">{v.address.suburb} • {v.serviceType}</div>
                      </div>
                    </div>
                  </button>
                ))}
      ...
      {/* Center: Primary Navigation Tabs (Larger Text) */}
      <nav className="flex items-center space-x-1.5 bg-slate-100 p-1.5 rounded-2xl border border-slate-200">
        ...
        >
          ...
          <span>Register</span>
          {openOrdersCount > 0 && (
            <span className="bg-slate-900 text-white text-xs px-2 py-0.5 rounded-full font-mono font-bold">
              {openOrdersCount}
            </span>
          )}
        </button>
        ...
        {activeVenue.enableKds && (
          ...
          >
            ...
            {kdsOrdersCount > 0 && (
              <span className="bg-amber-100 text-amber-900 border border-amber-300 text-xs px-2 py-0.5 rounded-full font-mono font-bold">
                {kdsOrdersCount}
              </span>
            )}
          </button>
      ...
      {/* Right: Surcharges & Staff (Larger Text) */}
      <div className="flex items-center space-x-2.5">
        {/* Weekend Surcharge Button */}
        <button
          onClick={() => toggleWeekendSurcharge()}
          title="Toggle Australian 10% Weekend Surcharge"
          className={`flex items-center space-x-1.5 px-3 py-2 rounded-xl text-xs sm:text-sm font-bold border transition ${
            isWeekendActive
        ...
        {/* Live Aussie Clock */}
        <div className="hidden md:flex items-center space-x-1.5 text-sm font-mono font-bold text-slate-800 bg-slate-100 px-3 py-2 rounded-xl border border-slate-200">
        ...
        <button
          ...
          className="flex items-center space-x-2 bg-slate-100 hover:bg-slate-200 px-3 py-2 rounded-xl border border-slate-200 transition"
        >
          <div
