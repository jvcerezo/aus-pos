...
export const ProductGrid: React.FC = () => {
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-100/80 p-3.5 overflow-hidden border-r border-slate-200">
      {/* 1. Active Open Tickets Switcher Bar (Larger Text) */}
      <div className="bg-white border border-slate-200 rounded-xl p-2.5 mb-3 shadow-xs">
        <div className="flex items-center justify-between mb-2 px-1">
          ...
          <button
            ...
            className="flex items-center space-x-1 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-bold transition shadow-xs"
          >
        ...
        <div className="flex items-center space-x-2 overflow-x-auto pb-0.5">
          {openOrders.length === 0 ? (
            <div className="text-sm text-slate-400 py-1 px-1 italic">
            ...
            openOrders.map(order => {
              ...
              return (
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl border text-sm font-bold transition whitespace-nowrap flex items-center space-x-2 shrink-0 ${
                    isCurrent
      ...
      {/* 2. Top Filter Bar: Search + Dietary Badges (Larger Text) */}
      <div className="flex items-center justify-between gap-3 mb-2.5">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-white border border-slate-300 rounded-xl pl-10 pr-4 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:border-slate-500 shadow-xs font-medium"
          />
          {searchQuery && (
            <button
      ...
      {/* 3. Category Tabs (Larger Text) */}
      <div className="flex items-center space-x-2 overflow-x-auto pb-1.5 mb-2.5">
        <button
          onClick={() => {
            sounds.playTap();
            setSelectedCategory('all');
          }}
          className={`px-4 py-2 rounded-xl text-sm font-bold uppercase tracking-wider transition whitespace-nowrap border ${
      ...
      {/* 4. Realistic Touch Register Product Grid (Larger Bolder Tiles) */}
            ...
            {venueItems.map(item => {
              ...
              return (
                <button
                  ...
                  className={`relative p-3.5 rounded-2xl border border-b-2 text-left flex flex-col justify-between transition transform active:translate-y-0.5 min-h-[125px] bg-white border-slate-200 hover:border-slate-400 shadow-xs ${
                    item.isAvailable
                ...
                >
                  ...
                  <div>
                    ...
                    {hasModifiers && (
                      <span className="inline-block mt-1 text-xs bg-slate-100 text-slate-700 border border-slate-200 px-2 py-0.5 rounded-md font-semibold">
                        Options ▾
                      </span>
                    )}
                  </div>
